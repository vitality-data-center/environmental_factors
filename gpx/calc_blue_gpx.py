...
def update_all_ndvi(records, buffer):
    ...
    try:
        ...
        cur_update.executemany("UPDATE " +target_table+"_"+ str(buffer) + " SET ndvi_blue = %s where id = %s",
        records)
        conn_update.commit()
...
def calc_ndvi_avg(id, geom_txt, buffer):
    ...
    if total_num == 0:
        logger.info("ways gid = "+ str(id) + " has no green value!")
        return id, ndvi_avg    
...
def calc(table, buffer):
    ...
    cur, conn = uf.connect()    
    cur.execute("select id, ST_AsText(ST_Transform(linestring, 28992)) from " + source_table +" where visits_r is not null or visits_b is not null or visits_w is not null;")
    ...
    for row in record_result_set:
        ...
        result_one_tuple=()
        id, ndvi_avg = calc_ndvi_avg(id, geom_txt, buffer)
        ...
        if row_index % uf.batch_number == 0:
            ...
            print("calc ndvi blue for id ", int(id), "row_index = " , row_index, " result= " , result_one_tuple, "; running time = ", time_diff_min, " mins! ", time_diff, " sec! ", " buffer = ", buffer)
            logger.info("calc ndvi blue for id "+ str(int(id))+  " row_index = " + str(row_index) + " result= " +  str(result_one_tuple) + "; running time = " +  str(time_diff_min) + " mins! " + str(time_diff) + " sec! buffer = " + str(buffer))
