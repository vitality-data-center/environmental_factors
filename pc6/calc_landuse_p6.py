...
def calc_landuse_index(pc6_id, center_geom_txt, buffer):
    ...
    cur_1.execute("""with clip_query as (SELECT ST_ValueCount(ST_Clip(st_union(rast), clipper.geom, false)) as pvc
     ...
     SELECT (pvc).value, SUM((pvc).count) As total from clip_query  GROUP BY (pvc).value;""", ([center_geom_txt, buffer]))
...
def calc(table, buffer):
    ...
    cur, conn = uf.connect()    
    #cur.execute("select gid, ST_AsText(st_centroid(geom)) from " + source_table +" where gid = 7943;")
